        ...
        for action in actions:
            if action == [1, 1]:
                if level <= H:  # H_NO_ATTACK
                    ...
                    print(
                        f"55 act:{action} level:{level}\033[91m point: {tmp_response.value}\033[00m",
                ...
                print(
                    f"75 action:{action} level:{level}\033[91m point: {tmp_response.value}\033[00m",
                    response.value,
                    tmp_response.act_list,
                    tmp_response.pos_list,
                )
                if response.value < tmp_response.value:
                    response = tmp_response
            print(f"80 return max_val level:{level}\033[91m value: {response.value}\033[00m", response.act_list)
            ...
            for act_atk in WeaponRange.WOODEN.value:
                ...
                if base_map.get_obj_map(pos_w_atk) == 3:
                    ...
                    if response.value < tmp_response.value:
                        ...
                        pr_red(f"set expect pos {response.expect_pos} expect face {response.expect_face}")
